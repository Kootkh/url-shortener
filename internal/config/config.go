...
type Config struct {
	...
	*
	*	Такие тэги добавляются после типа поля в определении структуры и представляют собой строку, заключенную в обратные кавычки.
	*
	...
	*		формат: yaml, json, jsonb, xml и др.
	...
	*		структуры и поля из файла конфига, а затем проверить что все на месте, расставить дефолтные значения и т.п.
	*
	*	`
...
// MustLoad - функция, которая прочитает файл с конфигом и заполнит созданные нами объекты
//
// Приставка "Must" в названии функции используется (по соглашению) когда функция вместо "возврата ошибки" должна использовать метод "паники", т.е. падение приложения. Так делать стоит в редких случаях - "Инициализация конфига" один из таких случаев!
//
// Принимает: None.
//
// Возвращает: *Config - указатель на объект конфига.
func MustLoad() *Config {

	// откуда читаем конфиг? В данном случае - из переменной окружения
